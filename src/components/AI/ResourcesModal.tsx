...
interface ServiceView { id: ServiceId; label: string; note?: string; members: ServiceId[]; status: 'stopped' | 'running' | 'busy'; pid: number | null; rssBytes: number | null; estimateBytes: number; measured: boolean; lastUsedAt: number | null; idleMinutes: number; managed: boolean; canStart: boolean; canStop: boolean; displacedBy: ServiceId | null }
...
/**
 * 本机资源：哪几组模型在跑、各占多少内存、多久没用了，手动启停，设空闲多久自动停。
 * 一起启停的算一组（嵌入跟着对话）；转写是用的时候自己起、用完自己退，不给开关。
 * 内存那一行三项加起来正好是总内存（本机模型 + 其它程序 + 可用），不然看着像算错了
 */
const ResourcesModal: React.FC<Props> = ({ onClose }) => {
  ...
  // 三项加起来正好是总内存：本机模型 + 其它程序 + 可用
  const others = state ? Math.max(0, state.totalBytes - state.availableBytes - used) : 0;
            ...
            <>
              <div className="lm-card" style={{ marginBottom: 12 }}>
                ...
                <div className="lm-kv">
                  ...
                  <div><span className="lm-kv__k">其它程序</span><span className="lm-kv__v">约 {fmtGB(others)}</span></div>
                    ...
                    <div className="res-row__side">
                      {s.managed ? (
                        <>
                          ...
                          {s.canStop && <button className="btn btn-secondary btn-sm" disabled={pending === s.id} onClick={() => void act(s.id, 'stop')}><PowerOff size={12} /> 停止</button>}
                          {/* 生图正常是出图时自己起，这个按钮只是想先把模型读进内存的人用的 */}
                          {s.canStart && <button className="btn btn-secondary btn-sm" disabled={pending === s.id} onClick={() => void act(s.id, 'start')}><Play size={12} /> {s.id === 'image' ? '预热' : '启动'}</button>}
                        </>
                      ) : <span className="res-row__auto">用时自动启动</span>}
                    </div>
                  </div>
                ))}
              </div>
