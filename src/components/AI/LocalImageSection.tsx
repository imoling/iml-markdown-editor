...
export const LocalImageSection: React.FC<Props> = ({ cfg, update }) => {
  ...
  const eta = formatDuration(estimateMs(sizeOf(cfg.localSize), stepsOf(cfg.localSteps), st.lastRun, { includeModelLoad: st.server.status !== 'running' }));
  ...
  return (
    <section className="lm-section">
      ...
      <div className="lm-card" style={{ marginTop: 10 }}>
        ...
        <div className="lm-line">按这个设置，一张图{st.lastRun ? '大约要 ' : '预计 '}<strong>{eta}</strong>{st.lastRun ? `（上一张实测 ${formatDuration(st.lastRun.ms)}，已按这台电脑的速度折算）` : '（含第一次加载模型的时间）'}。</div>
        <div className="lm-line lm-line--muted">快慢主要看步数：每一步的开销和分辨率几乎无关，步数翻倍时间就翻倍，降分辨率省不了多少、却明显掉细节。出图途中可以取消：那张图不要了，生图服务会一起停掉（下次出图重新加载模型，多花半分钟）。</div>
      </div>
    </section>
  );
};
