...
| 文件 | 规格 | 用在哪 |
|---|---|---|
| `media/demo-vertical.mp4` | 1080×1920 竖屏 · 2 分钟 · 5.6 MB | 抖音、视频号、小红书 |
| `media/demo-landscape.mp4` | 1920×1080 横屏 · 2 分钟 · 5.8 MB | B 站、V2EX、YouTube、官网 |
...
**内容顺序**（写 → 攒 → 问 → 听）：片头 →
**写**：普通 .md 文件 → 双模切换 → 斜杠菜单 → 折叠 → 本机大模型写作助手 → 自动续写（Tab 收下）→ 说一句配图 →
单机多模型按内存灵活调度 →
**攒**：`[[` 链接与悬浮预览 → 命令面板 → 闪念随手记 `⌃⌥N`（浏览器前台呼出小窗记三条）→ 点日历看当天日记 →
**问**：问你的笔记 → **听**：转写（麦克风收现场 / 系统声音收网课）→ 轻量版 Lite → 片尾。
配音是微软「晓晓」音色（edge-tts）+ 自己合成的背景音乐（无版权，说话时自动压低），整体 −14 LUFS。
